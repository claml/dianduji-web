...
**里程碑 5（当前）**：TXT 阅读、整句翻译、自定义释义、句子收藏、主题。
- 打开 PDF 或 **TXT**（滚动段落阅读，UTF-8/GB18030 自动识别）；PDF 自动加载
  `public/demo.pdf` 演示点读；
- 工具栏：打开、缩放、翻页、页码跳转、**目录**、**收藏**、**设置**；
- **词典**：ECDICT 6 万词条 + 专业词典（5 领域 1065 条，多词术语识别）；
- **词卡**：音标/词性/双语释义 + 领域标签 + **自定义释义**（未收录词可自写，
  持久化优先于在线翻译）+ **原文与整句翻译**（网关）+ **收藏句子**；
- **收藏**：生词与句子两个页签（IndexedDB 持久化）；
- **在线翻译**：词库未收录时兜底（设置中可配网关地址/开关，默认
  ...
  免费方案）后填公网地址即可线上使用；
- **阅读记忆**：每个文件记住进度（PDF 页码 / TXT 滚动位置），重新打开续读；
- **主题**：跟随系统 / 日间 / 夜间；
